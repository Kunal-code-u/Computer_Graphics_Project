...
void drawScene() { // morning
    glClearColor(1.0f, 1.0f, 1.0f, 1.0f); //white background when screen is cleared
    glClear(GL_COLOR_BUFFER_BIT);
    glColor3d(1, 0, 0);// default color unless changed
    glLoadIdentity(); //Clear any previous Transformation that might be saved
    gluOrtho2D(-12, 38, -19, 14);//range of the window
    glMatrixMode(GL_MODELVIEW);  //tells to prepare for transformations 
    ...
    glPushMatrix();
    glColor3d(255, 255, 0);
    glTranslatef(30.0, 7.0, 0);  // cordiantes are fixed so not moving but is needed for glutSolidSphere function as it is by default at orgin
    ...
    glTranslatef(_moveC, 0.0f, 0.0f);  //responsible for cloud movement
    glPushMatrix();
    ...
    glRotatef(_angle1, 0.0f, 0.0f, 1.0f);  // z cordinate, so it rotates in sy plane
    ...
    glutSwapBuffers();// for GLUT_DOUBLE, shows after everything works, not realtime
}


void init() {// initial state of the OpenGL rendering environment before rendering begin and we will get black screen after GLUT buffer clears
...
void SpecialInput(int key, int x, int y) // day and night
{
    switch (key)
    {
    case GLUT_KEY_UP:
    ...
    glutPostRedisplay(); // redraw or update the screen
}



void update1(int value) { // boat 1 motion
    ...
    // If the object goes too far(off the screen), reset its position back to the start.

//_moveA += 0.11;

    glutTimerFunc(20, update1, 0); //Updates the Update 1 function 
    glutPostRedisplay();
}

void handleMouse(int button, int state, int x, int y) { //using mouse to increase or decrease speed
...
void handleKeypress(unsigned char key, int x, int y) { // using keyboard to increase or decrease speed
    switch (key) {
    case 's'://stops
        speed = 0.0f;
        break;
...
void update2(int value) { //boat 2 motion
    ...
    glutTimerFunc(20, update2, 0); //updates update2 periodically. 0 is a parameter passed.
    glutPostRedisplay();
}

void update3(int value) {//function for windlmill
    ...
    glutTimerFunc(10, update3, 0); //update
}
void update4(int value) {//cloud motion
    ...
    glutTimerFunc(20, update4, 0); //update
    glutPostRedisplay();
}
...
int main(int argc, char** argv) {
    glutInit(&argc, argv); // initalise GLUT library and prepare the GLUT to handle 
    glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGB); // double buffering (remove flickering) and RGB mode
    glutInitWindowSize(1500, 1000);
    glutCreateWindow("Transformation");
    init();
    glutSpecialFunc(SpecialInput); // takes special characters for day and night
    glutDisplayFunc(drawScene);// tells to apply drawscene funtion 
    glutTimerFunc(20, update1, 0); //initiate moves boat 1; call periodically
    glutTimerFunc(20, update2, 0); //initiate moves boat 2; call periodically

    glutTimerFunc(10, update3, 0);// initiate moves windmill
    glutTimerFunc(20, update4, 0);// initiate moves cloud
    glutKeyboardFunc(handleKeypress); //run boat1 using keyboard
    glutMouseFunc(handleMouse); // run boat2 using mouse
    glutMainLoop();  // handles all user input and main function
    return 0;
}
